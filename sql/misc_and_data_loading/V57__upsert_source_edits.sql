...
/* Upsert a Source */
CREATE OR ALTER PROCEDURE dbo.upsert_source
(
    @name        NVARCHAR(50),
    @type        NVARCHAR(15),
...
BEGIN
  ...
  USING (SELECT
            ...
            @exclude_ps    AS url_exclude_patterns
         ) AS src
  ...
  WHEN MATCHED THEN
    UPDATE SET
      ...
      tgt.is_enabled           = COALESCE(@is_enabled, tgt.is_enabled)   -- don't change if NULL
  WHEN NOT MATCHED THEN
    INSERT (source_name, source_type, source_base_url, source_schema_url,
            ...
            url_base_exclude, url_exclude_patterns, is_enabled)
    VALUES (src.name, src.type, src.base_url, src.schema_url,
            src.crawl_depth, src.include_ext, src.page_timeout, src.max_conc,
            src.url_base_exclude, src.url_exclude_patterns, COALESCE(@is_enabled, 1));  -- default=1
